            ...
            if ($model->load($request->post())) {
                ...
                if(empty($model->id)){
                    if (Member::findOne(['username'=>$model->username])){
                        throw new \Exception("This account is exists");
                    }
                }

                if ($model->save()) {
                    //添加员工
                    $model["member"] = ArrayHelper::toArray($model["member"]);
                    if (!Employee::findOne($model["member"]['id'])){
                        $employee = new Employee();
                        $employee->id = $model["member"]['id'];
                        $employee->account = $model->username;
                        $employee->employee = $model->username;
                        $employee->joined_at = date("Y-m-d");
                        if (!$employee->save()) {
                            throw new \Exception($this->getError($employee));
                        }
                    }
